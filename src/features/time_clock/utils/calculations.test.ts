...
import { calculateWorkBlocks, calculateTotalDuration, formatDuration } from './calculations'
import type { TimeRecord } from '../../../types/pocketbase-types'

describe('Time Clock Calculations', () => {
    const mockRecord = (id: string, type: 'start' | 'finish', timestamp: string): TimeRecord => ({
        id,
        type,
        timestamp,
        user: 'user-1',
